...
def load():
    ...
    clf_path = os.path.join(
        model_base_path, "pretrained/guardnet_classifier.pkl")
    vect_path = os.path.join(
        model_base_path, "pretrained/guardnet_vectorizer.pkl")

    if os.path.exists(clf_path) and os.path.exists(vect_path):
        print("Pre-trained model found. Loading model")
        classifier = GuardMLP.load(vect_path, clf_path)
    else:
